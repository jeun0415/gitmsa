...
-- 2. 이름의 첫 글자가 대문자인 사원의 정보를 출력하세요.??? between A to Z
select *
from employees
...
having min(length(street_address) = (select length(street_address) from locations);
...
select first_name, last_name, hire_date, curdate(), year(curdate())-year(hire_date)
from employees;
