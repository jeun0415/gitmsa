...
--
-- Temporary view structure for view `vw_ord`
--

DROP TABLE IF EXISTS `vw_ord`;
/*!50001 DROP VIEW IF EXISTS `vw_ord`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `vw_ord` AS SELECT 
 1 AS `name`,
 1 AS `bookname`,
 1 AS `orderid`,
 1 AS `custid`,
...
--
-- Final view structure for view `vw_ord`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_ord` AS select `c`.`name` AS `name`,`b`.`bookname` AS `bookname`,`o`.`orderid` AS `orderid`,`o`.`custid` AS `custid`,`o`.`bookid` AS `bookid`,`o`.`saleprice` AS `saleprice`,`o`.`orderdate` AS `orderdate` from ((`customer` `c` join `orders` `o`) join `book` `b`) where ((`c`.`custid` = `o`.`custid`) and (`b`.`bookid` = `o`.`bookid`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
